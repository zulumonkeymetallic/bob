 ...
 * ALSO fabricated a batch of untracked sub-tasks per story via LLM — it had silently
 * created 1,300+ junk tasks in production before being removed. This job only ever
 * writes acceptanceCriteria (never creates tasks), runs once nightly rather than on
 * every keystroke-adjacent write, and is scoped to stories in the user's ACTIVE
 * sprint(s) — persona-scoped, so a personal story is only "in sprint" against a
 * personal active sprint, matching the persona-awareness fix elsewhere in this file.
 */
async function generateMissingAcceptanceCriteria() {
  const db = ensureFirestore();
  const profiles = await db.collection('profiles').get().catch(() => ({ docs: [] }));
    ...
    try {
      ...
      for (const chunk of chunks) {
        const storiesSnap = await db.collection('stories')
          ...
          .get()
          .catch((err) => {
            console.warn(`[AcceptanceCriteria] story query failed for user ${userId}:`, err?.message || err);
            return { docs: [] };
          });
        storiesSnap.docs.forEach((doc) => {
          const data = doc.data() || {};
      ...
      for (const story of scoped) {
        ...
        writer.set(story.ref, {
          ...
          aiMetadata: {
            ...
            reasoning: 'Auto-generated by nightly acceptance-criteria job',
          },
          updatedAt: admin.firestore.FieldValue.serverTimestamp(),
        }, { merge: true });
        generated++;
      ...
      if (generated > 0) console.log(`[AcceptanceCriteria] user ${userId}: generated for ${generated} stories`);
    } catch (err) {
