...
// NOTE: per-story acceptance-criteria generation used to run synchronously on every
// stories/{id} write (onStoryWrite -> enrichStory) and ALSO fabricated a batch of
// sub-tasks per story via LLM. That silently generated 1,300+ untracked tasks on
// production (status:'todo' string, no persona/type — never matched any other task
// creation path) and fired on every single story ever saved without acceptance
// criteria. Removed. Acceptance-criteria generation is now a scheduled, sprint-scoped
// job (generateMissingAcceptanceCriteria in nightlyOrchestration.js, runs before
// priority scoring) and no longer creates any tasks.
