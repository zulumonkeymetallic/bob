...
const ChoresTasksPage: React.FC = () => {
  ...
  const [form, setForm] = useState<{ title: string; type: TaskType; repeatFrequency: string; repeatInterval: number; daysOfWeek: string; tags: string; timeOfDay: '' | 'morning' | 'afternoon' | 'evening' }>(
    { title: '', type: 'chore', repeatFrequency: 'weekly', repeatInterval: 1, daysOfWeek: 'mon,tue,wed,thu,fri', tags: '', timeOfDay: '' }
  );
  ...
  const openCreate = () => {
    setEditing(null);
    setForm({ title: '', type: 'chore', repeatFrequency: 'weekly', repeatInterval: 1, daysOfWeek: 'mon,tue,wed,thu,fri', tags: '', timeOfDay: '' });
    setShowModal(true);
  };
          ...
          <Modal.Body>
            <Row className="g-3">
              ...
              <Col md={5}>
                ...
                <Form.Select value={form.timeOfDay} onChange={(e) => setForm({ ...form, timeOfDay: e.target.value as any })}>
