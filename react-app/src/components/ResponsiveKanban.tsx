...
const ResponsiveKanban: React.FC = () => {
  ...
  const [editTask, setEditTask] = useState({
    title: '',
    description: '',
    effort: 'medium' as 'S' | 'M' | 'L' | 'medium',
    priority: 'medium' as 'high' | 'med' | 'low' | 'medium',
  ...
  const handleEditTask = (task: Task) => {
    ...
    const mappedStatus = task.status === 'todo' ? 'planned' : 
                        task.status === 'in-progress' ? 'in_progress' : 
                        task.status === 'blocked' ? 'planned' : 
                        task.status;
    ...
    setEditTask({
      ...
      status: mappedStatus as 'planned' | 'in_progress' | 'done'
    });
  ...
  const handleSaveTaskEdit = async () => {
    if (!currentUser || !editTask.title.trim() || !selectedTask) return;
            ...
            <Row>
              ...
              <Col md={4}>
                <Form.Group className="mb-3">
                  <Form.Label>Effort</Form.Label>
                  <Form.Select
                    value={editTask.effort}
                    onChange={(e) => setEditTask({...editTask, effort: e.target.value as any})}
                  >
