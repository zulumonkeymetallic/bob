...
const TaskTemplate: React.FC<{ data: GanttTask }> = ({ data }) => {
  ...
  if (data.isMilestone) {
    return (
      <div className="grv6-task-shell grv6-milestone-shell" title={label}>
        <div className="grv6-task-label-outside grv6-milestone-label" style={{ color: data.themeColor }}>
          {label}
        </div>
        <div
  ...
  return (
    <div className="grv6-task-shell" title={label}>
      <div className="grv6-task-label-outside" style={{ color: data.themeColor }}>
        {label}
      </div>
      <div
        className="grv6-task"
      ...
      >
        <div className="grv6-progress-outer">
          ...
          <div className="grv6-progress-overlay">
            ...
            {data.pointsPct !== undefined && data.storyPoints !== undefined && (
...
const GoalRoadmapV6: React.FC = () => {
  ...
  const [zoomPercent, setZoomPercent] = useState<number>(70); // 10..100 mapped to levels
    ...
    switch (zoomLevel) {
      case 'year':
        return [
          ...
          { unit: 'month', step: 1, format: monthNarrow }
        ];
      case 'quarter':
        return [
          { unit: 'year', step: 1, format: 'yyyy' },
  ...
  const { tasks, chartStart, chartEnd } = useMemo(() => {
    ...
    const endDate = max ? new Date(max.getTime() + 14 * DAY_MS) : new Date(today.getTime() + 90 * DAY_MS);
    return { tasks: list, chartStart: startDate, chartEnd: endDate };
  }, [sortedGoals, globalThemes, storyDonePoints, storyPoints, potBalances, goalsById, handleScheduleGoal, handleGenerateStories, showSidebar]);
  ...
  const percentToLevel = useCallback((pct: number): typeof zoomLevel => {
    if (pct <= 25) return 'year';
    if (pct <= 55) return 'quarter';
    if (pct <= 85) return 'month';
    return 'week';
  }, []);
    ...
    <div className={`grv6-container theme-${theme}`}>
      ...
      <div className="grv6-body">
        <div className="grv6-main">
          ...
          {loading ? (
            <div className="grv6-loading">Loading roadmap...</div>
          ) : (
