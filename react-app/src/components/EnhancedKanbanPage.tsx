...
const EnhancedKanbanPage: React.FC = (): JSX.Element => {
  ...
  const swimLanes = [
    ...
    { id: 'active', title: 'In Progress', status: 2, color: '#3b82f6' },
    { id: 'done', title: 'Done', status: 4, color: '#059669' }
  ];
  ...
  const handleAddStory = async () => {
    ...
    try {
      await addDoc(collection(db, 'stories'), {
        ...
        status: 0,
        priority: parseInt(newStory.priority.slice(1)),
  ...
  const handleUpdateStory = async () => {
    ...
    try {
      await updateDoc(doc(db, 'stories', selectedStory.id), {
        ...
        priority: parseInt(editStory.priority.slice(1)),
  ...
  const openEditStory = (story: Story, e: React.MouseEvent) => {
    ...
    setEditStory({
      ...
      goalId: story.goalId || '',
      priority: `P${story.priority}` as 'P1' | 'P2' | 'P3',
      ...
      >
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '8px' }}>
          ...
          <div style={{ display: 'flex', gap: '4px', flexShrink: 0 }}>
            ...
            <span style={{
              backgroundColor: story.priority === 1 ? '#fecaca' : story.priority === 2 ? '#fed7aa' : '#d1fae5',
              color: story.priority === 1 ? '#991b1b' : story.priority === 2 ? '#9a3412' : '#065f46',
              padding: '2px 8px',
              borderRadius: '12px',
              fontSize: '11px',
              fontWeight: '500',
    ...
    <Container style={{ maxWidth: '100%', padding: '0 20px' }}>
      ...
      {goals.length === 0 && (
        <div style={{
          ...
          <Button variant="warning" href="/goals">
