#!/usr/bin/env node
/*
  Run task deduplication now for a given user by calling the deployed callable
  `deduplicateTasks` using a custom token minted from a service account.

  Usage:
    node scripts/run-dedupe-now.js --uid <UID> [--serviceAccount /abs/path/to/sa.json] [--hardDelete] [--dryRun] [--includeTitleDedupe=true]

  Default service account path: ~/.secrets/bob/bob20250810-service-account.json
*/
...
try {
  ...
  // Fallback to functions' node_modules if running from repo root
...
async function main() {
  ...
  // Use Admin SDK to mint a custom token for the target UID
  ...
  if (!signInRes.ok) {
    ...
    throw new Error(`verifyCustomToken failed: ${signInRes.status} ${text}`);
  }
  const signInJson = await signInRes.json();
  const idToken = signInJson.idToken;
  if (!idToken) throw new Error('No idToken in sign-in response');

  // Call the callable function over HTTPS using the onCall protocol
  const fnUrl = `https://${region}-${projectId}.cloudfunctions.net/deduplicateTasks`;
  const fnRes = await fetch(fnUrl, {
    ...
    body: JSON.stringify({ data: { hardDelete, dryRun, includeTitleDedupe } }),
  });
  if (!fnRes.ok) {
    const text = await fnRes.text();
    throw new Error(`deduplicateTasks call failed: ${fnRes.status} ${text}`);
  }
