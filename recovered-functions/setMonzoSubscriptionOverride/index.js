...
const { recordIntegrationLog, recordAiLog } = require('./utils/logging');
const { generateGoalResearch, buildStoryPlanFromResearch } = require('./lib/goalWorkflow');
...
Object.assign(exports, require('./monzo/classification'));
...
const INCOME_MIN_THRESHOLD = 15000; // £150 in minor units
...
let cachedMonzoEncryptionKey = null;
function getMonzoEncryptionKey() {
  if (cachedMonzoEncryptionKey) return cachedMonzoEncryptionKey;
  const raw = MONZO_TOKEN_ENCRYPTION_KEY.value?.() || process.env.MONZO_TOKEN_ENCRYPTION_KEY;
  if (!raw) {
    throw new Error('MONZO_TOKEN_ENCRYPTION_KEY not configured');
  }
  let keyBuffer;
  try {
  ...
  if (keyBuffer.length !== 32) {
    throw new Error('Monzo token encryption key must resolve to 32 bytes');
  }
  cachedMonzoEncryptionKey = keyBuffer;
  return cachedMonzoEncryptionKey;
}

function encryptTokenValue(plainText) {
  if (!plainText) return null;
  const key = getMonzoEncryptionKey();
...
function decryptTokenValue(payload) {
...
async function storeOauthState(provider, uid, nonce) {
  const db = admin.firestore();
  const value = `${provider}:${uid}:${nonce}`;
  const hashed = crypto.createHash('sha256').update(value).digest('hex');
  const ref = db.collection('oauth_states').doc(hashed);
  const expiresAt = admin.firestore.Timestamp.fromDate(new Date(Date.now() + OAUTH_STATE_TTL_MS));
  await ref.set({
    provider,
    uid,
    nonceHash: hashed,
...
async function consumeOauthState(provider, uid, nonce) {
  const db = admin.firestore();
  const value = `${provider}:${uid}:${nonce}`;
  const hashed = crypto.createHash('sha256').update(value).digest('hex');
  const ref = db.collection('oauth_states').doc(hashed);
  ...
  const data = snap.data() || {};
  if (data.provider !== provider || data.uid !== uid) {
    throw new Error('OAuth state mismatch');
  }
  try {
    await ref.delete();
  } catch (error) {
    console.warn('Failed to delete OAuth state', { provider, uid, error: error?.message || error });
  }
}
...
exports.monzoListAccounts = httpsV2.onCall({ secrets: [MONZO_CLIENT_ID, MONZO_CLIENT_SECRET, MONZO_TOKEN_ENCRYPTION_KEY] }, async (req) => {
  const uid = req?.auth?.uid; if (!uid) throw new httpsV2.HttpsError('unauthenticated', 'Sign in required');
...
exports.monzoSyncTransactions = httpsV2.onCall({ secrets: [MONZO_CLIENT_ID, MONZO_CLIENT_SECRET, MONZO_TOKEN_ENCRYPTION_KEY] }, async (req) => {
  const uid = req?.auth?.uid; if (!uid) throw new httpsV2.HttpsError('unauthenticated', 'Sign in required');
...
exports.monzoOAuthStart = httpsV2.onRequest({ secrets: [MONZO_CLIENT_ID], invoker: 'public' }, async (req, res) => {
  try {
    ...
    const nonce = String(req.query.nonce || "");
    if (!uid || !nonce) return res.status(400).send("Missing uid/nonce");
    await storeOauthState('monzo', uid, nonce);
...
exports.monzoOAuthCallback = httpsV2.onRequest({ secrets: [MONZO_CLIENT_ID, MONZO_CLIENT_SECRET, MONZO_TOKEN_ENCRYPTION_KEY], invoker: 'public' }, async (req, res) => {
  try {
    const code = String(req.query.code || "");
    const state = stateDecode(req.query.state);
    const uid = state.uid;
    const nonce = state.nonce;
    if (!code || !uid || !nonce) return res.status(400).send("Missing code/uid/nonce");
    try {
      await consumeOauthState('monzo', uid, nonce);
    } catch (error) {
      console.error('Monzo OAuth state validation failed', error);
    ...
    const db = admin.firestore();
    const encryptedRefresh = encryptTokenValue(refresh);
    const encryptedAccess = encryptTokenValue(access);
...
async function ensureMonzoAccessToken(uid) {
  ...
  try {
    if (data.access_token_enc) {
      accessToken = decryptTokenValue(data.access_token_enc);
    } else if (data.access_token) {
      accessToken = data.access_token;
      updates.access_token_enc = encryptTokenValue(accessToken);
      updates.access_token = admin.firestore.FieldValue.delete();
      shouldUpdate = true;
    }
  } catch (error) {
    console.error('Failed to decrypt Monzo access token', { uid, error: error?.message || error });
    throw new httpsV2.HttpsError('internal', 'Unable to decrypt stored Monzo access token');
  }

  try {
    if (data.refresh_token_enc) {
      refreshToken = decryptTokenValue(data.refresh_token_enc);
    } else if (data.refresh_token) {
      refreshToken = data.refresh_token;
      updates.refresh_token_enc = encryptTokenValue(refreshToken);
      updates.refresh_token = admin.firestore.FieldValue.delete();
      shouldUpdate = true;
    }
  } catch (error) {
    console.error('Failed to decrypt Monzo refresh token', { uid, error: error?.message || error });
    throw new httpsV2.HttpsError('internal', 'Unable to decrypt stored Monzo refresh token');
  }
  ...
  if (!needsRefresh) {
    return { accessToken, tokenRef, tokenData: { ...data, access_token: accessToken, refresh_token: refreshToken, expires_at: expiresAt } };
  }
  ...
  if (!newAccessToken) {
    throw new httpsV2.HttpsError('internal', 'Monzo refresh token exchange failed');
  }
  const expiresIn = Number(refreshed.expires_in || 0);
  expiresAt = expiresIn ? nowSeconds + expiresIn : null;

  await tokenRef.set({
    access_token_enc: encryptTokenValue(newAccessToken),
    refresh_token_enc: encryptTokenValue(newRefreshToken),
    access_token: admin.firestore.FieldValue.delete(),
    refresh_token: admin.firestore.FieldValue.delete(),
    expires_at: expiresAt,
  ...
  return {
    ...
    tokenRef,
    tokenData: { ...data, access_token: newAccessToken, refresh_token: newRefreshToken, expires_at: expiresAt },
  };
}
...
exports.monzoRegisterWebhook = httpsV2.onCall({ secrets: [MONZO_CLIENT_ID, MONZO_CLIENT_SECRET, MONZO_TOKEN_ENCRYPTION_KEY] }, async (req) => {
  if (!req || !req.auth) throw new httpsV2.HttpsError('unauthenticated', 'Sign in required.');
  const uid = req.auth.uid;
...
exports.monzoWebhook = httpsV2.onRequest({ secrets: [MONZO_CLIENT_ID, MONZO_CLIENT_SECRET, MONZO_TOKEN_ENCRYPTION_KEY, MONZO_WEBHOOK_SECRET] }, async (req, res) => {
  try {
    if (req.method !== 'POST') return res.status(405).send('Method not allowed');
    const webhookSecret = MONZO_WEBHOOK_SECRET.value?.() || process.env.MONZO_WEBHOOK_SECRET || '';
    ...
    if (typeof req.body === 'object' && req.body !== null) {
      ...
      try {
        ...
        try { await admin.firestore().collection('webhook_logs').add({ source: 'monzo', direction: 'in', ts: Date.now(), warn: 'invalid json', error: String(parseError?.message || parseError) }); } catch {}
...
exports.monzoBackstopSync = schedulerV2.onSchedule({ schedule: 'every 15 minutes', secrets: [MONZO_CLIENT_ID, MONZO_CLIENT_SECRET, MONZO_TOKEN_ENCRYPTION_KEY] }, async () => {
  const db = admin.firestore();
...
exports.syncMonzo = httpsV2.onCall({ secrets: [MONZO_CLIENT_ID, MONZO_CLIENT_SECRET, MONZO_TOKEN_ENCRYPTION_KEY] }, async (req) => {
  if (!req || !req.auth) throw new httpsV2.HttpsError('unauthenticated', 'Sign in required.');
  const uid = req.auth.uid;
...
exports.setMonzoSubscriptionOverride = httpsV2.onCall({ secrets: [MONZO_CLIENT_ID, MONZO_CLIENT_SECRET] }, async (req) => {
  if (!req || !req.auth) throw new httpsV2.HttpsError('unauthenticated', 'Sign in required.');
  const uid = req.auth.uid;
  const rawKey = String(req.data?.merchantKey || '').trim();
  const decision = String(req.data?.decision || '').trim().toLowerCase();
  if (!rawKey) {
    throw new httpsV2.HttpsError('invalid-argument', 'merchantKey is required');
  }
  if (!['keep', 'reduce', 'cancel'].includes(decision)) {
    throw new httpsV2.HttpsError('invalid-argument', 'decision must be keep, reduce, or cancel');
  }
  const note = req.data?.note ? String(req.data.note).trim() : null;
  const normalized = normaliseMerchantName(rawKey);
  const overridesRef = admin.firestore().collection('monzo_subscription_overrides').doc(uid);
  ...
  await computeMonzoAnalytics(uid);
  try { await recordIntegrationLog(uid, 'monzo', 'success', 'Updated subscription override', { merchantKey: normalized, decision }); } catch {}
  return { ok: true };
});
...
async function syncMonzoDataForUser(uid, { since } = {}) {
  ...
  if (accounts.length) {
    ...
    for (const account of accounts) {
      try {
        const balanceData = await monzoApi(accessToken, '/balance', { account_id: account.id });
        const balanceUpdate = {
          balanceMinor: Number.isFinite(Number(balanceData?.balance)) ? Number(balanceData.balance) : null,
          totalBalanceMinor: Number.isFinite(Number(balanceData?.total_balance)) ? Number(balanceData.total_balance) : null,
          balanceCurrency: balanceData?.currency || account.currency || null,
          balanceRetrievedAt: admin.firestore.FieldValue.serverTimestamp(),
        };
        await db.collection('monzo_accounts').doc(`${uid}_${account.id}`).set(balanceUpdate, { merge: true });
      } catch (error) {
        console.warn('monzo: balance fetch failed', { uid, accountId: account.id, error: error?.message || error });
        try {
          await db.collection('webhook_logs').add({ source: 'monzo', direction: 'internal', ts: Date.now(), warn: 'balance_fetch_failed', accountId: account.id, error: String(error?.message || error) });
        } catch {}
      }
    }
  }
...
async function sendGoalResearchEmail(uid, goal, researchSnapshot) {
  try {
    const db = ensureFirestore();
    const profileSnap = await db.collection('profiles').doc(uid).get();
    if (!profileSnap.exists) return false;
    const profile = profileSnap.data() || {};
    const email = profile.email || profile.contactEmail || null;
    if (!email) return false;

    const questions = Array.isArray(researchSnapshot?.questions) ? researchSnapshot.questions : [];
    const docMd = researchSnapshot?.docMd || '';
    const goalLink = goal?.ref ? `/goals/${goal.ref}` : `/goals/${goal?.id || ''}`;
    const html =
      `<h2>Research Brief: ${escapeHtml(goal?.title || 'Goal')}</h2>` +
      (questions.length
        ? `<p><strong>Key Questions</strong></p><ul>${questions.map((q) => `<li>${escapeHtml(String(q))}</li>`).join('')}</ul>`
        : '') +
      `<hr/><div style="white-space:pre-wrap;font-family:system-ui,-apple-system,Segoe UI,Roboto,Helvetica,Arial;">${escapeHtml(docMd)}</div>` +
      `<p style="margin-top:16px;">Review in BOB: <a href="${goalLink}">${goalLink}</a></p>`;

    await sendEmail({
      to: email,
      subject: `Research Brief · ${goal?.title || 'Goal'}`,
      html,
    });
    try {
      const snippet = html.length > 800 ? `${html.slice(0, 800)}…` : html;
      await recordIntegrationLog(uid, 'email', 'success', 'Sent goal research email', {
        goalId: goal?.id || goal?.goalId || null,
        subject: `Research Brief · ${goal?.title || 'Goal'}`,
        html: snippet,
      });
    } catch (logErr) {
      console.warn('[sendGoalResearchEmail] failed to log email', logErr?.message || logErr);
    }
    return true;
  } catch (error) {
    console.warn('[sendGoalResearchEmail] failed', error?.message || error);
    return false;
  }
}

async function persistGoalResearch({
  goalRef,
  goal,
  uid,
  research,
  source = 'goal_auto_research',
  ...
  const researchDocPayload = {
    id: researchRef.id,
    ownerUid: uid,
    goalId: goalRef.id,
    title: `Research: ${goal?.title || goalRef.id}`,
    researchPrompt: research.researchPrompt,
    questions: research.questions,
    outline: research.outline,
    nextActions: research.nextActions,
    docMd: research.researchDocMd,
  ...
  await researchRef.set(researchDocPayload, { merge: true });

  const reviewStatus = goal?.reviewStatus === 'approved' ? 'approved' : 'pending';
  const workflowStatus = reviewStatus === 'approved' ? goal?.workflowStatus || 'ready' : 'review';

  const researchSnapshot = {
    docId: researchRef.id,
    prompt: research.researchPrompt,
    questions: research.questions,
    outline: research.outline,
    nextActions: research.nextActions,
    docMd: research.researchDocMd,
  ...
  await goalRef.set(
    {
      llmResearch: researchSnapshot,
      latestResearchDocId: researchRef.id,
      reviewStatus,
      workflowStatus,
  ...
  await activityRef.set({
    ...
    entityId: goalRef.id,
    entityType: 'goal',
    activityType: 'goal_research_generated',
    userId: uid,
    description: `AI generated deep research for goal ${goal?.title || goalRef.id}`,
    metadata: {
      researchDocId: researchRef.id,
      outline: research.outline?.length || 0,
      nextActions: research.nextActions?.length || 0,
      provider: research.provider,
      model: research.model,
      source,
    },
  ...
  if (sendEmailNotification) {
    emailSent = await sendGoalResearchEmail(uid, goal, { ...researchSnapshot, docMd: research.researchDocMd });
  }
...
async function createStoriesFromPlan({
  db,
  uid,
  goalId,
  goal,
  ...
  for (const storyPlan of plan.stories || []) {
    ...
    const theme = storyPlan.theme || goal?.theme || 'Growth';
    const storyPayload = {
      id: storyRef.id,
      ref: storyRefValue,
      ownerUid: uid,
      ...
      points: storyPlan.points || 3,
      orderIndex: Date.now(),
      entry_method: 'ai_goal_research',
      researchDocId,
      calendarSuggestion: storyPlan.calendarSuggestion || null,
      createdAt: now,
      updatedAt: now,
    };

    if (sprintId) {
      storyPayload.sprintId = sprintId;
      storyPayload.sprintAssignedAt = now;
    }

    batch.set(storyRef, storyPayload);
    storiesCreated.push({ id: storyRef.id, ref: storyRefValue, theme, calendarSuggestion: storyPlan.calendarSuggestion });

    for (const taskPlan of storyPlan.tasks || []) {
      const taskRef = db.collection('tasks').doc();
      const estimateMinutes = Math.max(15, Math.min(720, Number(taskPlan.estimateMinutes) || 60));
      const taskPayload = {
        id: taskRef.id,
        ownerUid: uid,
        persona,
        ...
        estimated_duration: estimateMinutes,
        entry_method: 'ai_goal_research',
        researchDocId,
        derivedFromResearch: taskPlan.derivedFromResearch === true,
        autoConversionSuggested: taskPlan.convertToStory === true,
        createdAt: Date.now(),
        updatedAt: Date.now(),
      };
      batch.set(taskRef, taskPayload);
      tasksCreated.push({ id: taskRef.id, storyId: storyRef.id, convertToStory: taskPlan.convertToStory === true });
    }
  }

  if (storiesCreated.length) {
    const activityRef = db.collection('activity_stream').doc();
    batch.set(activityRef, {
      id: activityRef.id,
      entityId: goalId,
      entityType: 'goal',
      activityType: 'goal_research_stories_created',
      userId: uid,
      description: `AI generated ${storiesCreated.length} stories and ${tasksCreated.length} tasks from research`,
      metadata: {
        storyIds: storiesCreated.map((s) => s.id),
        taskIds: tasksCreated.map((t) => t.id),
        researchDocId,
      },
      createdAt: now,
      updatedAt: now,
  ...
  if (storiesCreated.length || tasksCreated.length) {
    await batch.commit();
  }

  return { storiesCreated, tasksCreated };
}

exports.onGoalCreated = firestoreV2.onDocumentCreated('goals/{goalId}', { secrets: [GOOGLE_AI_STUDIO_API_KEY, NYLAS_API_KEY] }, async (event) => {
  const snap = event.data;
  if (!snap) return;
  const goal = snap.data() || {};
  const uid = goal.ownerUid || goal.userId;
  if (!uid) return;
  if (goal.skipAutoResearch === true) return;
  if (!goal.title) return;
  if (!goal.llmResearch && goal.reviewStatus === 'approved') return;
  if (!event.params?.goalId) return;
  if (goal.llmResearch?.docMd && goal.reviewStatus) return;

  const provider = String(goal.researchProvider || goal.llmProvider || 'gemini');
  const model = String(goal.researchModel || goal.llmModel || 'gemini-1.5-flash');

  try {
    const research = await generateGoalResearch({
    ...
    const { researchDocId, emailSent } = await persistGoalResearch({
      goalRef: snap.ref,
      goal,
      uid,
      research,
    ...
    await recordAiLog(uid, 'goalAutoResearch', 'success', 'Goal research generated on creation', {
      goalId: snap.id,
      researchDocId,
      emailSent,
    });
  } catch (error) {
    console.error('[onGoalCreated] failed to generate research', error?.message || error);
    try {
      await recordAiLog(uid, 'goalAutoResearch', 'error', 'Failed to generate goal research on creation', {
        goalId: snap.id,
...
exports.orchestrateGoalPlanning = functionsV2.https.onCall({ secrets: [GOOGLE_AI_STUDIO_API_KEY, NYLAS_API_KEY] }, async (req) => {
  ...
  await recordAiLog(uid, 'orchestrateGoalPlanning', 'info', 'Goal orchestration started', {
    goalId,
    researchProvider,
    researchModel,
    planningProvider,
  ...
  const { researchDocId } = await persistGoalResearch({
    goalRef: goalSnap.ref,
    goal,
    uid,
    research,
    source: 'goal_orchestrate',
    sendEmailNotification: true,
  });

  if (researchOnly) {
    await recordAiLog(uid, 'orchestrateGoalPlanning', 'success', 'Goal research generated (research only)', {
      goalId,
      researchDocId,
    });
  ...
  const plan = buildStoryPlanFromResearch({ goal, research });
  const persona = goal?.persona || 'personal';

  const sprintId = await getPreferredSprintId(uid);
  const { storiesCreated, tasksCreated } = await createStoriesFromPlan({
    db,
    uid,
    goalId,
    goal,
  ...
  let appliedBlocks = 0;
  try {
    const context = await assemblePlanningContext(uid, persona, 3);
    const aiBlocks = await generateAIPlan(context, {
      focusGoalId: goalId,
      llmProvider: planningProvider,
      llmModel: planningModel,
    });
    const validation = await validateCalendarBlocks(aiBlocks.blocks || [], context);
    if (validation.errors.length === 0) {
      appliedBlocks = await applyCalendarBlocks(
        uid,
        persona,
        (aiBlocks.blocks || []).map((block) => ({
          ...block,
          goalId,
          entry_method: 'calendar_ai_goal',
          confidence_score: validation.score,
        }))
      );
    } else {
      console.warn('[orchestrateGoalPlanning] calendar validation errors', validation.errors);
    }
  } catch (error) {
    console.warn('[orchestrateGoalPlanning] scheduling failed', error?.message || error);
  }

  await goalSnap.ref.set(
    {
      reviewStatus: 'approved',
      workflowStatus: 'ready',
      status: typeof goal.status === 'number' && goal.status < 1 ? 1 : goal.status || 1,
      llmResearch: {
        ...(goal.llmResearch || {}),
        docId: researchDocId,
        storyIds: storiesCreated.map((s) => s.id),
        generatedStoriesAt: admin.firestore.FieldValue.serverTimestamp(),
        provider: research.provider,
        model: research.model,
      },
      updatedAt: admin.firestore.FieldValue.serverTimestamp(),
  ...
  await recordAiLog(uid, 'orchestrateGoalPlanning', 'success', 'Goal research, stories, tasks generated and scheduled', {
    goalId,
    researchDocId,
    storiesCreated: storiesCreated.length,
...
async function loadGoalResearchDetails({ db, goal }) {
  let research = goal.llmResearch || null;
  if (!research || !research.docMd) {
    const docId = research?.docId || goal.latestResearchDocId;
    if (!docId) return null;
    try {
      const researchDocSnap = await db.collection('research_docs').doc(docId).get();
      if (researchDocSnap.exists) {
        const data = researchDocSnap.data() || {};
        research = Object.assign({}, research || {}, {
          docId,
          docMd: data.docMd || research?.docMd || '',
          outline: data.outline || research?.outline || [],
          nextActions: data.nextActions || research?.nextActions || [],
          questions: data.questions || research?.questions || [],
          prompt: data.researchPrompt || research?.prompt || '',
          provider: data.provider || research?.provider || 'gemini',
          model: data.model || research?.model || 'gemini-1.5-flash',
        });
      }
    } catch (error) {
      console.warn('[loadGoalResearchDetails] unable to load research doc', error?.message || error);
    }
  }
  return research;
}

async function approveGoalResearchInternal({
  uid,
  goalId,
  schedule = true,
  force = false,
  planningProvider = 'gemini',
  planningModel = 'gemini-1.5-flash',
}) {
  const db = ensureFirestore();
  const goalSnap = await db.collection('goals').doc(goalId).get();
  if (!goalSnap.exists) throw new httpsV2.HttpsError('not-found', 'Goal not found');
  ...
  if (
    !force &&
    goal.reviewStatus === 'approved' &&
    Array.isArray(goal.llmResearch?.storyIds) &&
    goal.llmResearch.storyIds.length > 0
  ) {
    return {
      status: 'already-approved',
      goalId,
      researchDocId: goal.llmResearch?.docId || goal.latestResearchDocId || null,
      storiesCreated: 0,
      tasksCreated: 0,
      appliedBlocks: 0,
      sprintId: goal.llmResearch?.sprintId || null,
      planSummary: goal.llmResearch?.planSummary || null,
    };
  }

  const research = await loadGoalResearchDetails({ db, goal });
  if (!research || !research.docMd) {
    throw new httpsV2.HttpsError('failed-precondition', 'Goal research is required before approval');
  }

  const plan = buildStoryPlanFromResearch({ goal, research });
  const persona = goal?.persona || 'personal';
  const sprintId = await getPreferredSprintId(uid);
  const researchDocId = research.docId || goal.latestResearchDocId || null;
  ...
  if (schedule) {
    try {
      const context = await assemblePlanningContext(uid, persona, 3);
      const aiPlan = await generateAIPlan(context, {
        focusGoalId: goalId,
        llmProvider: planningProvider,
        llmModel: planningModel,
      });
      ...
      if (validation.errors.length === 0) {
        ...
        console.warn('[approveGoalResearch] calendar validation errors', validation.errors);
      }
    } catch (error) {
      console.warn('[approveGoalResearch] scheduling failed', error?.message || error);
    }
  }
  ...
  await recordAiLog(uid, 'approveGoalResearch', 'success', 'Goal research approved and stories generated', {
    goalId,
    researchDocId,
    storiesCreated: storiesCreated.length,
    tasksCreated: tasksCreated.length,
...
exports.approveGoalResearch = functionsV2.https.onCall({ secrets: [GOOGLE_AI_STUDIO_API_KEY, NYLAS_API_KEY] }, async (req) => {
  const uid = req?.auth?.uid;
  if (!uid) throw new httpsV2.HttpsError('unauthenticated', 'Sign in required');
  ...
  const result = await approveGoalResearchInternal({
    uid,
    goalId,
    force: req?.data?.force === true,
    schedule: req?.data?.schedule !== false,
    planningProvider: String(req?.data?.planningProvider || 'gemini'),
    planningModel: String(req?.data?.planningModel || 'gemini-1.5-flash'),
  });

  return { ok: true, ...result };
});

exports.approveAllGoalResearch = functionsV2.https.onCall({ secrets: [GOOGLE_AI_STUDIO_API_KEY, NYLAS_API_KEY] }, async (req) => {
  const uid = req?.auth?.uid;
  if (!uid) throw new httpsV2.HttpsError('unauthenticated', 'Sign in required');

  const schedule = req?.data?.schedule !== false;
  const planningProvider = String(req?.data?.planningProvider || 'gemini');
  const planningModel = String(req?.data?.planningModel || 'gemini-1.5-flash');
  const limit = Math.max(1, Math.min(Number(req?.data?.limit || 5), 15));

  const db = ensureFirestore();
  const pendingSnap = await db.collection('goals')
    .where('ownerUid', '==', uid)
    .where('reviewStatus', '==', 'pending')
    .limit(limit)
    .get();

  const workflowSnap = await db.collection('goals')
    .where('ownerUid', '==', uid)
    .where('workflowStatus', '==', 'review')
    .limit(limit)
    .get();

  const goalIds = new Set();
  pendingSnap.docs.forEach((doc) => goalIds.add(doc.id));
  workflowSnap.docs.forEach((doc) => goalIds.add(doc.id));

  if (goalIds.size === 0) {
    return { ok: true, processed: 0, approved: 0, results: [] };
  }

  const results = [];
  for (const goalId of Array.from(goalIds).slice(0, limit)) {
    try {
      const result = await approveGoalResearchInternal({
        uid,
        goalId,
      ...
      results.push({ goalId, status: result.status, storiesCreated: result.storiesCreated, tasksCreated: result.tasksCreated });
    } catch (error) {
      console.warn('[approveAllGoalResearch] failed for goal', goalId, error?.message || error);
      results.push({ goalId, status: 'error', error: error?.message || String(error) });
      await recordAiLog(uid, 'approveGoalResearch', 'error', 'Bulk approval failed for goal', {
        goalId,
        error: error?.message || String(error),
      });
    }
  ...
  await recordAiLog(uid, 'approveGoalResearchBulk', approved > 0 ? 'success' : 'warning', 'Bulk goal research approval run', {
...
async function applyCalendarBlocks(uid, persona, blocks) {
  ...
  for (const proposed of blocks) {
    ...
    const themeLabel = enriched.theme || 'Growth';
    const baseTitle = enriched.title || proposed.title || `${proposed.category || 'Block'} (${themeLabel})`;
    const decoratedTitle = baseTitle.startsWith('[') ? baseTitle : `[${themeLabel}] ${baseTitle}`;
    const deepLink = enriched.linkUrl || proposed.linkUrl || null;
    const notesParts = [
      proposed.notes || null,
      deepLink ? `Deep link: ${deepLink}` : null,
      proposed.rationale ? `AI rationale: ${proposed.rationale}` : null,
    ].filter(Boolean);
    ...
    let linkType = null, refId = null, linkTitle = null, linkUrl = deepLink;
