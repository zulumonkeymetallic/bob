...
const { recordIntegrationLog } = require('./utils/logging');
    ...
    for (const userDoc of usersSnapshot.docs) {
      ...
      try {
        ...
        if (userProfile.email && userProfile.emailDigestEnabled !== false) {
          ...
          try {
            const snippet = digestContent && digestContent.length > 1200 ? `${digestContent.slice(0, 1200)}…` : digestContent;
            await recordIntegrationLog(userId, 'dailyDigestEmail', 'success', 'Daily digest email sent', {
              email: userProfile.email,
              html: snippet,
              metrics: digestDoc.metrics,
            });
          } catch (logErr) {
            console.warn('[dailyDigest] failed to log email content', logErr?.message || logErr);
          }
